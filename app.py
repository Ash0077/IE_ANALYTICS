...
def processRequest(req):
    dbConn = pymongo.MongoClient('mongodb://admin:test!_2o20@52.37.152.224:27017/')  # opening a connection to Mongo
    ...
    if intent == 'signup_search':
        cust_country = parameters.get("IE-signups")
        all1, family = makeAPIRequest(cust_country)
        print(makeAPIRequest(cust_country))
        print(all1['all'])
        webhookresponse = "***Signup Report*** \n\n" + " Overall signup CSY :" +str(all1['all']) + \
                          "\n"  + "\n" + " Family signup CSY : " + str(family['family'])+  "\n" 
        log.saveConversations(sessionID, cust_country, webhookresponse, intent, db)
      
        log.saveCases( "country", all1, db)
    ...
    if intent == 'covid_searchcountry':
        ...
        print(webhookresponse,"weeeeeee")
        log.saveConversations(sessionID, cust_country, webhookresponse, intent, db)
...
def configureDataBase():
    ...
    return client.get_database('compass')
...
def makeAPIRequest(query):
    ...
    if query == "overall":
        return api.makeApiRequestForsignup(query)
